...
function Input_Digit(digit) {
    ...
    //Display_Value to the key that was clickd.
    if (Wait_Second_Operand === true) {
        Calculator.Display_Value = digit;
        Calculator.Wait_Second_Operand = false;
    } else {
        // this overwrites Display_Value if the current value is 0
        //otherwise it addsonto it
...
function Input_Decimal(dot) {
    ...
    if (!Calculator.Display_Value.includes(dot)) {
        //we are staying that if the Display_Value does not contain a decimal point
...
function Handle_Operator(Next_Operator) {
    ...
    //when an operator key is passed, we convert the current number
    ...
    if (First_Operand == null) {
        ...
        // this will remove any trailin 0's
...
// this function update the screen with the contets of Display_Value
